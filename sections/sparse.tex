...
\begin{frame}{Minimized, constrained, or regularized problem ?}
    \begin{tikzpicture}[remember picture,overlay]
        \begin{scope}[xshift=0.5\textwidth]
            ...
            %
            \onslide<2-> {
                \node[text width=0.35\textwidth,align=center] (loss) at (-3,1) {\textcolor{TolLightOrange}{$\lfunc(\pv)$} \\ thing to minimize};
                %
            ...
            %
            \onslide<4-> {
                \draw [ultra thick,dashed,<->] ($(loss)+(1.5,0)$) -- ($(norm)+(-1.5,0)$) node[midway,below,text width=0.25\linewidth,align=center,font=\small] {optim. problem};
            }
            %
            %
            %
    ...
    \begin{tikzpicture}[remember picture,overlay]
        \begin{scope}[xshift=0.5\textwidth]
            ...
            \onslide<2-> {
                ...
                \draw[ultra thick,->] ($(origin)+(0,-0.1)$) -- ($(origin)+(0,1.25)$) node[above,font=\small] {\textbf{1d graph}};
            ...
            %
            \onslide<3-> {
                \node[font=\small,text width=0.5\linewidth,align=center,anchor=north] at (origin.south) {non-convex, non-diff., \\ non-continuous, ...};
            }
            %
            %
            %
...
\begin{frame}{Topic of this talk}
    \begin{tikzpicture}[remember picture,overlay]
        \begin{scope}[xshift=0.5\linewidth]
            \onslide<1-> {
                \node[text width=0.4\linewidth,align=center,draw,very thick,rounded corners=0.25cm] at (-3,2.5) {Ok, $\ell_0$-problems can be of practical interest !};
                %
            ...
            %
            \onslide<3-> {
                \node[text width=0.75\linewidth,align=center] at (0,-0.75) {\textbf{1) MIP-based methods} \\ Convenient for practitioners \\ Poor numerical performances};
            }
            %
            %
            %
            \onslide<4-> {
                \node[text width=0.75\linewidth,align=center] at (0,-2.5) {\textbf{2) Specialized Branch-and-Bound} \\ More sophisticated mechanism \\ Better numerical performances};
            }
            %
            %
            %
            \onslide<5-> {
                \node[text width=0.75\linewidth,align=center,TolLightOrange] at (0,-3.75) {\textbf{High-level concepts and practical tools}};
            }
        \end{scope}
    \end{tikzpicture}
\end{frame}